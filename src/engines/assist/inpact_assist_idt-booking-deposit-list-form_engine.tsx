...
`,
  ...
  },
  {
    ...
    type: "question",
    phase: "Step 1 of 7",
    paal: `Every step from here on adds to one function. Write and export a function named \`DepositDesk\` that returns \`<div />\`.

Your task: define and export DepositDesk as a function component returning <div />.`,
    hint: `export function DepositDesk() {
  return <div />;
}`,
  ...
  return <div />;
    ...
    mc_options: ["export function DepositDesk() { return <div />; }","Write the JSX first, then wrap it in a function later","Skip the function — a component can be a bare object of props"],
    mc_correct_option: "export function DepositDesk() { return <div />; }",
    mc_anchor: "export function DepositDesk() { return <",
    why_this_matters: `A React component is just a function that returns JSX — naming and exporting that shell first is what lets every later step (and a real pull request) attach to something. Taking a deposit is the same list+form skill as booking — different fields, same React pattern you will reuse on every product desk.`,
    answer_keywords: ["export","function","DepositDesk","return"],
...
`,
    ...
    expected: `export function DepositDesk() {
  return <div />;
}
`,
  ...
  Submit   →  the new row appears on the list
`,
      discover: `export function DepositDesk() {
  return <div />;
}
`,
      ...
      build: `export function DepositDesk() {
  return <div />;
}`,
    },
  },
...
Your task: write \`type Deposit\` with \`id\` plus client, amount, appointmentId.`,
    hint: `Write a TypeScript type named Deposit with id plus: client, amount, appointmentId. Every row also needs id — the mock never shows it, but the list and the API both key off it.`,
...
Every value with a shape needs one type to describe that shape — and a list screen renders many values of the exact same shape, over and over. Looking at the mock row above, what does that shared type need to name — including a field the mock never shows on screen at all?`,
    mc_options: ["Build one type for a single Deposit row (id, client, amount, appointmentId)","Lock every screen to final copy and branding before modeling how one row looks","Wait until every backend endpoint exists before rendering any UI"],
    mc_correct_option: "Build one type for a single Deposit row (id, client, amount, appointmentId)",
    mc_anchor: "Build one type for a single Deposit row ",
    ...
    answer_keywords: ["export","type","Deposit","client","amount","appointmentId"],
    seed_code: `export function DepositDesk() {
  return <div />;
}

// Describe one list item. The list and form will use this type.
`,
    starter_code: `export function DepositDesk() {
  return <div />;
}
...
`,
      ...
      watchOut: "Do not turn a single import or interface into its own lesson.",
      dryRun: "Write the same step for a different resource with the same shape.",
      build: `Write a TypeScript type named Deposit with id plus: client, amount, appointmentId. Every row also needs id — the mock never shows it, but the list and the API both key off it.`,
    },
  },
  {
    id: "step3",
...
Your task: hold deposits in state that React watches, typed as Deposit[], starting empty.`,
    hint: `useState<Deposit[]>([]). Import useState.`,
    example_code: `const [guests, setGuests] = useState<Guest[]>([]);`,
    think_prompt: `\`\`\`text
LIST — Deposits
  Priya
  40   (grows by one every time take deposit is used)
\`\`\`
...
React only redraws a component when the value it reads changes through React itself — a plain variable can change without React ever finding out. Where should this growing array live so the screen actually redraws every time a row is added?`,
    mc_options: ["const [deposits, setDeposits] = useState<Deposit[]>([]);","let deposits = [];","const deposits = fetch('/api');"],
    mc_correct_option: "const [deposits, setDeposits] = useState<Deposit[]>([]);",
    mc_anchor: "const [deposits, setDeposits] = useState",
    why_this_matters: `A plain array in a variable will not make React redraw — useState is the list the screen actually watches. Taking a deposit is the same list+form skill as booking — different fields, same React pattern you will reuse on every product desk.`,
    answer_keywords: ["useState","deposits","setDeposits","Deposit"],
...
`,
      ...
      watchOut: "Do not turn a single import or interface into its own lesson.",
      dryRun: "Write the same step for a different resource with the same shape.",
      build: `useState<Deposit[]>([]). Import useState.`,
    },
  },
  {
    id: "step4",
...
Your task: now that you have the data as an array you can iterate over, return the JSX that renders each item as a row with key={item.id}.`,
    hint: `deposits.map((a) => <li key={a.id}>...</li>)`,
    example_code: `return (
  <ul>
    {guests.map((g) => (
...
Turning an array into UI is a mapping operation — one array item becomes one rendered element — and React needs a stable identifier per element to tell rows apart across re-renders. You already have the array in state. How do you turn it into the stacked rows shown above?`,
    ...
    why_this_matters: `map() walks the array and returns one element per item; a stable key tells React which row is which when the list changes. Taking a deposit is the same list+form skill as booking — different fields, same React pattern you will reuse on every product desk.`,
    answer_keywords: ["map","key","a.id","deposits"],
    seed_code: `import { useState } from "react";
...
`,
      ...
      watchOut: "Do not turn a single import or interface into its own lesson.",
      dryRun: "Write the same step for a different resource with the same shape.",
      build: `deposits.map((a) => <li key={a.id}>...</li>)`,
    },
  },
  {
    id: "step5",
...
Your task: render the empty message above when deposits.length === 0, and the mapped rows otherwise.`,
    hint: `deposits.length === 0 ? <p>No deposits yet.</p> : <ul>...</ul>`,
...
An array with zero items is a valid, common state — code that only knows how to map rows renders nothing at all when the array is empty, with no explanation for the user. When should the UI show the empty message above instead of the (empty) list?`,
    ...
    why_this_matters: `An empty list should not look broken — a clear empty state tells the user they can add the first row. Taking a deposit is the same list+form skill as booking — different fields, same React pattern you will reuse on every product desk.`,
...
export function DepositDesk() {
  ...
  return <ul>{deposits.map((a) => <li key={a.id}>{a.client}</li>)}</ul>;
}
`,
    starter_code: `import { useState } from "react";
...
`,
      ...
      watchOut: "Do not turn a single import or interface into its own lesson.",
      dryRun: "Write the same step for a different resource with the same shape.",
      build: `deposits.length === 0 ? <p>No deposits yet.</p> : <ul>...</ul>`,
    },
  },
  {
    id: "step6",
