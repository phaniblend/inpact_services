...
Your task: declare \`let reminders = []\` and a \`nextId()\` helper backed by a counter (not Date.now()).`,
    hint: `let reminders = []; let nextIdCounter = 1;
...
A server that must remember data across requests needs one place to keep it that persists between calls, plus a way to generate a new identifier every time a record is created. Where do the rows above live between one request and the next, and how does each new row get an id nothing else already has?`,
    ...
    why_this_matters: `Due vs scheduled must be derived from sendAt and sent — not invented by the browser. One store is the single source of truth for list and create — every ScheduledReminder row created through /api/scheduled-reminders has to still be there on the next GET, or the desk would be lying about what's actually owed, booked, or open.`,
...
`,
      ...
      build: `let reminders = []; let nextIdCounter = 1;
function nextId() { return String(nextIdCounter++); }`,
    },
  },
  {
...
Your task: write validateScheduledReminder(input) that returns an error string when a required field is missing/invalid, else null.`,
    hint: `validateScheduledReminder returns error string or null.`,
...
Whatever a client sends in a request body can never be assumed well-formed — checking for missing or malformed fields before anything else runs is what keeps bad data from ever reaching storage. What must be true about the fields above before you insert a row?`,
    ...
    why_this_matters: `Validation keeps junk out of the store before status is ever derived from it. Due vs scheduled must be derived from sendAt and sent — not invented by the browser.`,
...
`,
      ...
      watchOut: "Do not turn a single import or interface into its own lesson.",
      dryRun: "Write the same step for a different resource with the same shape.",
      build: `validateScheduledReminder returns error string or null.`,
    },
  },
  {
    id: "step3",
...
Your task: write deriveReminderStatus(row, now) that returns the status string computed from the rule above — never from req.body.`,
    hint: `deriveReminderStatus(row, now) returns the status string.`,
...
A status label describing a record can always be recalculated from that record's own stored facts — comparing dates, or checking a boolean flag — rather than being sent by the client and simply trusted. Given the rule above, should the browser send status, or should the server compute it — and from what?`,
    ...
    why_this_matters: `The browser can send anything; computing status from facts you already trust is what keeps it from being faked. Due vs scheduled must be derived from sendAt and sent — not invented by the browser.`,
...
`,
      ...
      watchOut: "Do not turn a single import or interface into its own lesson.",
      dryRun: "Write the same step for a different resource with the same shape.",
      build: `deriveReminderStatus(row, now) returns the status string.`,
    },
  },
  {
    id: "step4",
...
Your task: wire list() to map deriveReminderStatus over reminders, and create() to validate, push, then return the row with deriveReminderStatus attached.`,
    hint: `list: reminders.map(r => ({...r, status: deriveReminderStatus(r)}))`,
...
Attaching a computed field to data on its way out of a route means running the derive function once per record, every time that record is returned — never once at write time and then reused. How do GET and POST above reuse validateScheduledReminder and deriveReminderStatus to guarantee status is never stale?`,
    ...
    why_this_matters: `Always attach derive on the way out — POST validates, inserts, then attaches status, exactly like GET does. Due vs scheduled must be derived from sendAt and sent — not invented by the browser.`,
...
`,
      ...
      watchOut: "Do not turn a single import or interface into its own lesson.",
      dryRun: "Write the same step for a different resource with the same shape.",
      build: `list: reminders.map(r => ({...r, status: deriveReminderStatus(r)}))`,
    },
  },
];
