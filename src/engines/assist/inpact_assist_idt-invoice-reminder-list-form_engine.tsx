...
`,
  ...
  },
  {
    ...
    type: "question",
    phase: "Step 1 of 7",
    paal: `Every step from here on adds to one function. Write and export a function named \`ReminderDesk\` that returns \`<div />\`.

Your task: define and export ReminderDesk as a function component returning <div />.`,
    hint: `export function ReminderDesk() {
  return <div />;
}`,
  ...
  return <div />;
    ...
    mc_options: ["export function ReminderDesk() { return <div />; }","Write the JSX first, then wrap it in a function later","Skip the function — a component can be a bare object of props"],
    mc_correct_option: "export function ReminderDesk() { return <div />; }",
    mc_anchor: "export function ReminderDesk() { return ",
    why_this_matters: `A React component is just a function that returns JSX — naming and exporting that shell first is what lets every later step (and a real pull request) attach to something. Collections work is a second list+form: who gets nudged, how, and when.`,
    answer_keywords: ["export","function","ReminderDesk","return"],
...
`,
    ...
    expected: `export function ReminderDesk() {
  return <div />;
}
`,
  ...
  Submit   →  the new row appears on the list
`,
      discover: `export function ReminderDesk() {
  return <div />;
}
`,
      ...
      build: `export function ReminderDesk() {
  return <div />;
}`,
    },
  },
...
Your task: write \`type Reminder\` with \`id\` plus invoiceId, channel, sendAt.`,
    hint: `Write a TypeScript type named Reminder with id plus: invoiceId, channel, sendAt. Every row also needs id — the mock never shows it, but the list and the API both key off it.`,
...
Every value with a shape needs one type to describe that shape — and a list screen renders many values of the exact same shape, over and over. Looking at the mock row above, what does that shared type need to name — including a field the mock never shows on screen at all?`,
    mc_options: ["Build one type for a single Reminder row (id, invoiceId, channel, sendAt)","Lock every screen to final copy and branding before modeling how one row looks","Wait until every backend endpoint exists before rendering any UI"],
    mc_correct_option: "Build one type for a single Reminder row (id, invoiceId, channel, sendAt)",
    mc_anchor: "Build one type for a single Reminder row",
    ...
    answer_keywords: ["export","type","Reminder","invoiceId","channel","sendAt"],
    seed_code: `export function ReminderDesk() {
  return <div />;
}

// Describe one list item. The list and form will use this type.
`,
    starter_code: `export function ReminderDesk() {
  return <div />;
}
...
`,
      ...
      watchOut: "Do not turn a single import or interface into its own lesson.",
      dryRun: "Write the same step for a different resource with the same shape.",
      build: `Write a TypeScript type named Reminder with id plus: invoiceId, channel, sendAt. Every row also needs id — the mock never shows it, but the list and the API both key off it.`,
    },
  },
  {
    id: "step3",
...
Your task: hold reminders in state that React watches, typed as Reminder[], starting empty.`,
    hint: `useState<Reminder[]>([]). Import useState.`,
    example_code: `const [guests, setGuests] = useState<Guest[]>([]);`,
    think_prompt: `\`\`\`text
LIST — Reminders
  inv-9
  email   (grows by one every time schedule is used)
\`\`\`
...
React only redraws a component when the value it reads changes through React itself — a plain variable can change without React ever finding out. Where should this growing array live so the screen actually redraws every time a row is added?`,
    mc_options: ["const [reminders, setReminders] = useState<Reminder[]>([]);","let reminders = [];","const reminders = fetch('/api');"],
    mc_correct_option: "const [reminders, setReminders] = useState<Reminder[]>([]);",
    mc_anchor: "const [reminders, setReminders] = useSta",
    why_this_matters: `A plain array in a variable will not make React redraw — useState is the list the screen actually watches. Collections work is a second list+form: who gets nudged, how, and when.`,
    answer_keywords: ["useState","reminders","setReminders","Reminder"],
...
`,
      ...
      watchOut: "Do not turn a single import or interface into its own lesson.",
      dryRun: "Write the same step for a different resource with the same shape.",
      build: `useState<Reminder[]>([]). Import useState.`,
    },
  },
  {
    id: "step4",
...
Turning an array into UI is a mapping operation — one array item becomes one rendered element — and React needs a stable identifier per element to tell rows apart across re-renders. You already have the array in state. How do you turn it into the stacked rows shown above?`,
    ...
    why_this_matters: `map() walks the array and returns one element per item; a stable key tells React which row is which when the list changes. Collections work is a second list+form: who gets nudged, how, and when.`,
...
Your task: render the empty message above when reminders.length === 0, and the mapped rows otherwise.`,
    hint: `reminders.length === 0 ? <p>No reminders yet.</p> : <ul>...</ul>`,
...
An array with zero items is a valid, common state — code that only knows how to map rows renders nothing at all when the array is empty, with no explanation for the user. When should the UI show the empty message above instead of the (empty) list?`,
    ...
    why_this_matters: `An empty list should not look broken — a clear empty state tells the user they can add the first row. Collections work is a second list+form: who gets nudged, how, and when.`,
...
export function ReminderDesk() {
  ...
  return <ul>{reminders.map((a) => <li key={a.id}>{a.invoiceId}</li>)}</ul>;
}
`,
    starter_code: `import { useState } from "react";
...
`,
      ...
      watchOut: "Do not turn a single import or interface into its own lesson.",
      dryRun: "Write the same step for a different resource with the same shape.",
      build: `reminders.length === 0 ? <p>No reminders yet.</p> : <ul>...</ul>`,
    },
  },
  {
    id: "step6",
