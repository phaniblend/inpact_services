...
`,
  ...
  },
  {
    ...
    type: "question",
    phase: "Step 1 of 7",
    paal: `Every step from here on adds to one function. Write and export a function named \`UnansweredBoard\` that returns \`<div />\`.

Your task: define and export UnansweredBoard as a function component returning <div />.`,
    hint: `export function UnansweredBoard() {
  return <div />;
}`,
  ...
  return <div />;
    ...
    mc_options: ["export function UnansweredBoard() { return <div />; }","Write the JSX first, then wrap it in a function later","Skip the function — a component can be a bare object of props"],
    mc_correct_option: "export function UnansweredBoard() { return <div />; }",
    mc_anchor: "export function UnansweredBoard() { retu",
    why_this_matters: `A React component is just a function that returns JSX — naming and exporting that shell first is what lets every later step (and a real pull request) attach to something. Owners open a board of unanswered reviews first. Filter for display; keep full inbox in state.`,
    answer_keywords: ["export","function","UnansweredBoard","return"],
...
`,
    ...
    expected: `export function UnansweredBoard() {
  return <div />;
}
`,
  ...
  Filter   →  only matching rows render — the full list stays in state
`,
      discover: `export function UnansweredBoard() {
  return <div />;
}
`,
      ...
      build: `export function UnansweredBoard() {
  return <div />;
}`,
    },
  },
...
Your task: write \`type Review\` with \`id\` plus author, rating, status.`,
    hint: `Write a TypeScript type named Review with id plus: author, rating, status. Every row also needs id — the mock never shows it, but the list and the API both key off it.`,
...
Every value with a shape needs one type to describe that shape — and a list screen renders many values of the exact same shape, over and over. Looking at the mock row above, what does that shared type need to name — including a field the mock never shows on screen at all?`,
    mc_options: ["Build one type for a single Review row (id, author, rating, status)","Lock every screen to final copy and branding before modeling how one row looks","Wait until every backend endpoint exists before rendering any UI"],
    mc_correct_option: "Build one type for a single Review row (id, author, rating, status)",
    ...
    answer_keywords: ["export","type","Review","author","rating","status"],
    seed_code: `export function UnansweredBoard() {
  return <div />;
}

// Describe one list item. The list and form will use this type.
`,
    starter_code: `export function UnansweredBoard() {
  return <div />;
}
...
`,
      ...
      watchOut: "Do not turn a single import or interface into its own lesson.",
      dryRun: "Write the same step for a different resource with the same shape.",
      build: `Write a TypeScript type named Review with id plus: author, rating, status. Every row also needs id — the mock never shows it, but the list and the API both key off it.`,
    },
  },
  {
    id: "step3",
...
LIST — Needs reply
  Sam
  2   (grows by one every time log is used)
\`\`\`
...
React only redraws a component when the value it reads changes through React itself — a plain variable can change without React ever finding out. Where should this growing array live so the screen actually redraws every time a row is added?`,
    ...
    why_this_matters: `A plain array in a variable will not make React redraw — useState is the list the screen actually watches. Owners open a board of unanswered reviews first. Filter for display; keep full inbox in state.`,
...
Your task: render reviews.filter(...) mapped to rows — keep the full array in state, only narrow what's displayed.`,
    hint: `reviews.filter(...).map(...)`,
    example_code: `return (
  <ul>
...
LIST (filtered) — Needs reply
  Sam
  2   (only rows matching the current filter)
\`\`\`

Filtering for display means computing a smaller array from the full one with .filter() — the state array itself never loses any rows. How do you keep the complete reviews list in state but only render the subset above?`,
    ...
    why_this_matters: `Filtering in render (or with a derived array) lets users scan what matters without deleting other rows from state. Owners open a board of unanswered reviews first. Filter for display; keep full inbox in state.`,
    answer_keywords: ["filter","map","reviews"],
    seed_code: `import { useState } from "react";
...
Your task: render the empty message above when reviews.length === 0, and the mapped rows otherwise.`,
    hint: `reviews.length === 0 ? <p>No unanswered reviews.</p> : <ul>...</ul>`,
...
An array with zero items is a valid, common state — code that only knows how to map rows renders nothing at all when the array is empty, with no explanation for the user. When should the UI show the empty message above instead of the (empty) list?`,
    ...
    why_this_matters: `An empty list should not look broken — a clear empty state tells the user they can add the first row. Owners open a board of unanswered reviews first. Filter for display; keep full inbox in state.`,
...
`,
      ...
      watchOut: "Do not turn a single import or interface into its own lesson.",
      dryRun: "Write the same step for a different resource with the same shape.",
      build: `reviews.length === 0 ? <p>No unanswered reviews.</p> : <ul>...</ul>`,
    },
  },
  {
    id: "step6",
