...
Your task: declare \`let appointments = []\` and a \`nextId()\` helper backed by a counter (not Date.now()).`,
    hint: `let appointments = []; let nextIdCounter = 1;
...
A server that must remember data across requests needs one place to keep it that persists between calls, plus a way to generate a new identifier every time a record is created. Where do the rows above live between one request and the next in a simple lesson server, and how does each new row get an id nothing else already has?`,
    ...
    why_this_matters: `Service businesses cannot double-book a provider. The API must refuse overlapping slots so the calendar stays trustworthy. APIs need a single place to read and write; an array plus a real id generator is enough for the lesson, and the same shape carries over once appointments becomes a database table.`,
...
`,
      ...
      build: `let appointments = []; let nextIdCounter = 1;
function nextId() { return String(nextIdCounter++); }`,
    },
  },
  {
...
Your task: write validateAppointment(input) that returns an error string when a required field is missing/invalid, else null.`,
    hint: `validateAppointment(input) returns a string error or null.`,
...
Whatever a client sends in a request body can never be assumed well-formed — checking for missing or malformed fields before anything else runs is what keeps bad data from ever reaching storage. Looking at the POST body above, which checks have to pass before you push a row?`,
    ...
    why_this_matters: `Bad payloads must fail early with a clear error — validation is how /api/appointments protects the store. Service businesses cannot double-book a provider. The API must refuse overlapping slots so the calendar stays trustworthy.`,
...
`,
      ...
      watchOut: "Do not turn a single import or interface into its own lesson.",
      dryRun: "Write the same step for a different resource with the same shape.",
      build: `validateAppointment(input) returns a string error or null.`,
    },
  },
  {
    id: "step3",
...
SAMPLE — /api/appointments
Conflict rule: same provider + same startsAt is a conflict

Your task: write hasSlotConflict(candidate) that returns true when an existing row already matches the conflict rule above.`,
    hint: `hasSlotConflict(candidate) returns true when an existing row conflicts.`,
...
Two requests can describe the same resource at the same moment — before inserting one, you compare it against everything already stored to decide whether it collides. Given the rule above, how should the server decide "conflict" before insert?`,
    ...
    why_this_matters: `Clients can race — the server must refuse overlaps so the record stays trustworthy. Service businesses cannot double-book a provider. The API must refuse overlapping slots so the calendar stays trustworthy.`,
...
`,
      ...
      watchOut: "Do not turn a single import or interface into its own lesson.",
      dryRun: "Write the same step for a different resource with the same shape.",
      build: `hasSlotConflict(candidate) returns true when an existing row conflicts.`,
    },
  },
  {
    id: "step4",
...
Your task: wire list() to return appointments, and create() to call validateAppointment (400 on error), then hasSlotConflict (409 on conflict), then push and return 201.`,
    hint: `create: validate → overlap → push; list: res.json(appointments)`,
...
A route handler is where validation, conflict-checking, and storage come together — it should call the helpers you already wrote, in order, not re-implement any of their logic inline. How do GET and POST above use appointments, validateAppointment, and hasSlotConflict together?`,
    ...
    why_this_matters: `GET returns the store; POST validates (400), checks conflict (409), then inserts (201) — keep rules in helpers, not duplicated in the route. Service businesses cannot double-book a provider. The API must refuse overlapping slots so the calendar stays trustworthy.`,
...
`,
      ...
      watchOut: "Do not turn a single import or interface into its own lesson.",
      dryRun: "Write the same step for a different resource with the same shape.",
      build: `create: validate → overlap → push; list: res.json(appointments)`,
    },
  },
];
