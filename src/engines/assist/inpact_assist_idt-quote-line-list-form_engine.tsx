...
`,
  ...
  },
  {
    ...
    type: "question",
    phase: "Step 1 of 7",
    paal: `Every step from here on adds to one function. Write and export a function named \`QuoteLines\` that returns \`<div />\`.

Your task: define and export QuoteLines as a function component returning <div />.`,
    hint: `export function QuoteLines() {
  return <div />;
}`,
  ...
  return <div />;
    ...
    mc_options: ["export function QuoteLines() { return <div />; }","Write the JSX first, then wrap it in a function later","Skip the function — a component can be a bare object of props"],
    mc_correct_option: "export function QuoteLines() { return <div />; }",
    mc_anchor: "export function QuoteLines() { return <d",
    why_this_matters: `A React component is just a function that returns JSX — naming and exporting that shell first is what lets every later step (and a real pull request) attach to something. Estimates are built from lines. Same list+form skill — different nouns.`,
    answer_keywords: ["export","function","QuoteLines","return"],
...
`,
    ...
    expected: `export function QuoteLines() {
  return <div />;
}
`,
  ...
  Submit   →  the new row appears on the list
`,
      discover: `export function QuoteLines() {
  return <div />;
}
`,
      ...
      build: `export function QuoteLines() {
  return <div />;
}`,
    },
  },
...
Your task: write \`type QuoteLine\` with \`id\` plus quoteId, label, amount.`,
    hint: `Write a TypeScript type named QuoteLine with id plus: quoteId, label, amount. Every row also needs id — the mock never shows it, but the list and the API both key off it.`,
...
Every value with a shape needs one type to describe that shape — and a list screen renders many values of the exact same shape, over and over. Looking at the mock row above, what does that shared type need to name — including a field the mock never shows on screen at all?`,
    mc_options: ["Build one type for a single QuoteLine row (id, quoteId, label, amount)","Lock every screen to final copy and branding before modeling how one row looks","Wait until every backend endpoint exists before rendering any UI"],
    mc_correct_option: "Build one type for a single QuoteLine row (id, quoteId, label, amount)",
    mc_anchor: "Build one type for a single QuoteLine ro",
    ...
    answer_keywords: ["export","type","QuoteLine","quoteId","label","amount"],
    seed_code: `export function QuoteLines() {
  return <div />;
}

// Describe one list item. The list and form will use this type.
`,
    starter_code: `export function QuoteLines() {
  return <div />;
}
...
`,
      ...
      watchOut: "Do not turn a single import or interface into its own lesson.",
      dryRun: "Write the same step for a different resource with the same shape.",
      build: `Write a TypeScript type named QuoteLine with id plus: quoteId, label, amount. Every row also needs id — the mock never shows it, but the list and the API both key off it.`,
    },
  },
  {
    id: "step3",
...
Your task: hold lines in state that React watches, typed as QuoteLine[], starting empty.`,
    hint: `useState<QuoteLine[]>([]). Import useState.`,
    example_code: `const [guests, setGuests] = useState<Guest[]>([]);`,
    think_prompt: `\`\`\`text
LIST — Quote lines
  q-1
  Labor   (grows by one every time add line is used)
\`\`\`
...
React only redraws a component when the value it reads changes through React itself — a plain variable can change without React ever finding out. Where should this growing array live so the screen actually redraws every time a row is added?`,
    mc_options: ["const [lines, setLines] = useState<QuoteLine[]>([]);","let lines = [];","const lines = fetch('/api');"],
    mc_correct_option: "const [lines, setLines] = useState<QuoteLine[]>([]);",
    mc_anchor: "const [lines, setLines] = useState<Quote",
    why_this_matters: `A plain array in a variable will not make React redraw — useState is the list the screen actually watches. Estimates are built from lines. Same list+form skill — different nouns.`,
    answer_keywords: ["useState","lines","setLines","QuoteLine"],
...
`,
      ...
      watchOut: "Do not turn a single import or interface into its own lesson.",
      dryRun: "Write the same step for a different resource with the same shape.",
      build: `useState<QuoteLine[]>([]). Import useState.`,
    },
  },
  {
    id: "step4",
...
Your task: now that you have the data as an array you can iterate over, return the JSX that renders each item as a row with key={item.id}.`,
    hint: `lines.map((a) => <li key={a.id}>...</li>)`,
    example_code: `return (
  <ul>
    {guests.map((g) => (
...
Turning an array into UI is a mapping operation — one array item becomes one rendered element — and React needs a stable identifier per element to tell rows apart across re-renders. You already have the array in state. How do you turn it into the stacked rows shown above?`,
    ...
    why_this_matters: `map() walks the array and returns one element per item; a stable key tells React which row is which when the list changes. Estimates are built from lines. Same list+form skill — different nouns.`,
    answer_keywords: ["map","key","a.id","lines"],
    seed_code: `import { useState } from "react";
...
`,
      ...
      watchOut: "Do not turn a single import or interface into its own lesson.",
      dryRun: "Write the same step for a different resource with the same shape.",
      build: `lines.map((a) => <li key={a.id}>...</li>)`,
    },
  },
  {
    id: "step5",
...
Your task: render the empty message above when lines.length === 0, and the mapped rows otherwise.`,
    hint: `lines.length === 0 ? <p>No line items yet.</p> : <ul>...</ul>`,
...
An array with zero items is a valid, common state — code that only knows how to map rows renders nothing at all when the array is empty, with no explanation for the user. When should the UI show the empty message above instead of the (empty) list?`,
    ...
    why_this_matters: `An empty list should not look broken — a clear empty state tells the user they can add the first row. Estimates are built from lines. Same list+form skill — different nouns.`,
...
export function QuoteLines() {
  const [lines] = useState<QuoteLine[]>([]);
  return <ul>{lines.map((a) => <li key={a.id}>{a.quoteId}</li>)}</ul>;
}
`,
    starter_code: `import { useState } from "react";
...
`,
      ...
      watchOut: "Do not turn a single import or interface into its own lesson.",
      dryRun: "Write the same step for a different resource with the same shape.",
      build: `lines.length === 0 ? <p>No line items yet.</p> : <ul>...</ul>`,
    },
  },
  {
    id: "step6",
