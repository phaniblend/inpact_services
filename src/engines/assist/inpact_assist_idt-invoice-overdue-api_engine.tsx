...
Your task: declare \`let invoices = []\` and a \`nextId()\` helper backed by a counter (not Date.now()).`,
    hint: `let invoices = []; let nextIdCounter = 1;
...
A server that must remember data across requests needs one place to keep it that persists between calls, plus a way to generate a new identifier every time a record is created. Where do the rows above live between one request and the next, and how does each new row get an id nothing else already has?`,
    ...
    why_this_matters: `Overdue must be computed from due date and paid — never trusted from the client body. One store is the single source of truth for list and create — every Invoice row created through /api/invoices has to still be there on the next GET, or the desk would be lying about what's actually owed, booked, or open.`,
...
`,
      ...
      build: `let invoices = []; let nextIdCounter = 1;
function nextId() { return String(nextIdCounter++); }`,
    },
  },
  {
...
Your task: write validateInvoice(input) that returns an error string when a required field is missing/invalid, else null.`,
    hint: `validateInvoice returns error string or null.`,
...
Whatever a client sends in a request body can never be assumed well-formed — checking for missing or malformed fields before anything else runs is what keeps bad data from ever reaching storage. What must be true about the fields above before you insert a row?`,
    ...
    why_this_matters: `Validation keeps junk out of the store before status is ever derived from it. Overdue must be computed from due date and paid — never trusted from the client body.`,
...
`,
      ...
      watchOut: "Do not turn a single import or interface into its own lesson.",
      dryRun: "Write the same step for a different resource with the same shape.",
      build: `validateInvoice returns error string or null.`,
    },
  },
  {
    id: "step3",
...
Your task: write deriveStatus(row, now) that returns the status string computed from the rule above — never from req.body.`,
    hint: `deriveStatus(row, now) returns the status string.`,
...
A status label describing a record can always be recalculated from that record's own stored facts — comparing dates, or checking a boolean flag — rather than being sent by the client and simply trusted. Given the rule above, should the browser send status, or should the server compute it — and from what?`,
    ...
    why_this_matters: `The browser can send anything; computing status from facts you already trust is what keeps it from being faked. Overdue must be computed from due date and paid — never trusted from the client body.`,
...
`,
      ...
      watchOut: "Do not turn a single import or interface into its own lesson.",
      dryRun: "Write the same step for a different resource with the same shape.",
      build: `deriveStatus(row, now) returns the status string.`,
    },
  },
  {
    id: "step4",
...
Your task: wire list() to map deriveStatus over invoices, and create() to validate, push, then return the row with deriveStatus attached.`,
    hint: `list: invoices.map(r => ({...r, status: deriveStatus(r)}))`,
...
Attaching a computed field to data on its way out of a route means running the derive function once per record, every time that record is returned — never once at write time and then reused. How do GET and POST above reuse validateInvoice and deriveStatus to guarantee status is never stale?`,
    ...
    why_this_matters: `Always attach derive on the way out — POST validates, inserts, then attaches status, exactly like GET does. Overdue must be computed from due date and paid — never trusted from the client body.`,
...
`,
      ...
      watchOut: "Do not turn a single import or interface into its own lesson.",
      dryRun: "Write the same step for a different resource with the same shape.",
      build: `list: invoices.map(r => ({...r, status: deriveStatus(r)}))`,
    },
  },
];
