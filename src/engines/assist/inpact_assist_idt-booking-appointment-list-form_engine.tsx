...
`,
  ...
  },
  {
    ...
    type: "question",
    phase: "Step 1 of 7",
    paal: `Every step from here on adds to one function. Write and export a function named \`BookingDesk\` that returns \`<div />\`.

Your task: define and export BookingDesk as a function component returning <div />.`,
    hint: `export function BookingDesk() {
  return <div />;
}`,
  ...
  return <div />;
    ...
    mc_options: ["export function BookingDesk() { return <div />; }","Write the JSX first, then wrap it in a function later","Skip the function — a component can be a bare object of props"],
    mc_correct_option: "export function BookingDesk() { return <div />; }",
    mc_anchor: "export function BookingDesk() { return <",
    why_this_matters: `A React component is just a function that returns JSX — naming and exporting that shell first is what lets every later step (and a real pull request) attach to something. Most web apps need a screen that lists data as rows you can scan — people, orders, tickets. You also need a form to add a new row, and a clear message when the list is empty.`,
    answer_keywords: ["export","function","BookingDesk","return"],
...
`,
    ...
    expected: `export function BookingDesk() {
  return <div />;
}
`,
  ...
  Submit   →  the new row appears on the list
`,
      discover: `export function BookingDesk() {
  return <div />;
}
`,
      ...
      build: `export function BookingDesk() {
  return <div />;
}`,
    },
  },
...
Every value with a shape needs one type to describe that shape — and a list screen renders many values of the exact same shape, over and over. Looking at the mock row above, what does that shared type need to name — including a field the mock never shows on screen at all?`,
    ...
    seed_code: `export function BookingDesk() {
  return <div />;
}

// Describe one list item. The list and form will use this type.
`,
    starter_code: `export function BookingDesk() {
  return <div />;
}
...
LIST — Bookings
  Maya
  Color & cut   (grows by one every time book is used)
\`\`\`
...
React only redraws a component when the value it reads changes through React itself — a plain variable can change without React ever finding out. Where should this growing array live so the screen actually redraws every time a row is added?`,
    ...
    why_this_matters: `A plain array in a variable will not make React redraw — useState is the list the screen actually watches. Most web apps need a screen that lists data as rows you can scan — people, orders, tickets. You also need a form to add a new row, and a clear message when the list is empty.`,
...
Turning an array into UI is a mapping operation — one array item becomes one rendered element — and React needs a stable identifier per element to tell rows apart across re-renders. You already have the array in state. How do you turn it into the stacked rows shown above?`,
    ...
    why_this_matters: `map() walks the array and returns one element per item; a stable key tells React which row is which when the list changes. Most web apps need a screen that lists data as rows you can scan — people, orders, tickets. You also need a form to add a new row, and a clear message when the list is empty.`,
    answer_keywords: ["map","key","a.id","appointments"],
    seed_code: `import { useState } from "react";
...
`,
      ...
      watchOut: "Do not turn a single import or interface into its own lesson.",
      dryRun: "Write the same step for a different resource with the same shape.",
      build: `appointments.map((a) => <li key={a.id}>...</li>)`,
    },
  },
  {
    id: "step5",
...
Your task: render the empty message above when appointments.length === 0, and the mapped rows otherwise.`,
    hint: `appointments.length === 0 ? <p>No appointments yet.</p> : <ul>...</ul>`,
...
An array with zero items is a valid, common state — code that only knows how to map rows renders nothing at all when the array is empty, with no explanation for the user. When should the UI show the empty message above instead of the (empty) list?`,
    ...
    why_this_matters: `An empty list should not look broken — a clear empty state tells the user they can add the first row. Most web apps need a screen that lists data as rows you can scan — people, orders, tickets. You also need a form to add a new row, and a clear message when the list is empty.`,
...
`,
      ...
      watchOut: "Do not turn a single import or interface into its own lesson.",
      dryRun: "Write the same step for a different resource with the same shape.",
      build: `appointments.length === 0 ? <p>No appointments yet.</p> : <ul>...</ul>`,
    },
  },
  {
    id: "step6",
