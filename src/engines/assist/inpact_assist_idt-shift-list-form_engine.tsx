...
`,
  ...
  },
  {
    ...
    type: "question",
    phase: "Step 1 of 7",
    paal: `Every step from here on adds to one function. Write and export a function named \`ShiftBoard\` that returns \`<div />\`.

Your task: define and export ShiftBoard as a function component returning <div />.`,
    hint: `export function ShiftBoard() {
  return <div />;
}`,
  ...
  return <div />;
    ...
    mc_options: ["export function ShiftBoard() { return <div />; }","Write the JSX first, then wrap it in a function later","Skip the function — a component can be a bare object of props"],
    mc_correct_option: "export function ShiftBoard() { return <div />; }",
    mc_anchor: "export function ShiftBoard() { return <d",
    why_this_matters: `A React component is just a function that returns JSX — naming and exporting that shell first is what lets every later step (and a real pull request) attach to something. Small teams still run on group chats. A shift list+form is the staffing desk they can actually use.`,
    answer_keywords: ["export","function","ShiftBoard","return"],
...
`,
    ...
    expected: `export function ShiftBoard() {
  return <div />;
}
`,
  ...
  Submit   →  the new row appears on the list
`,
      discover: `export function ShiftBoard() {
  return <div />;
}
`,
      ...
      build: `export function ShiftBoard() {
  return <div />;
}`,
    },
  },
...
Your task: write \`type Shift\` with \`id\` plus worker, role, startsAt.`,
    hint: `Write a TypeScript type named Shift with id plus: worker, role, startsAt. Every row also needs id — the mock never shows it, but the list and the API both key off it.`,
...
Every value with a shape needs one type to describe that shape — and a list screen renders many values of the exact same shape, over and over. Looking at the mock row above, what does that shared type need to name — including a field the mock never shows on screen at all?`,
    mc_options: ["Build one type for a single Shift row (id, worker, role, startsAt)","Lock every screen to final copy and branding before modeling how one row looks","Wait until every backend endpoint exists before rendering any UI"],
    mc_correct_option: "Build one type for a single Shift row (id, worker, role, startsAt)",
    mc_anchor: "Build one type for a single Shift row (i",
    ...
    answer_keywords: ["export","type","Shift","worker","role","startsAt"],
    seed_code: `export function ShiftBoard() {
  return <div />;
}

// Describe one list item. The list and form will use this type.
`,
    starter_code: `export function ShiftBoard() {
  return <div />;
}
...
`,
      ...
      watchOut: "Do not turn a single import or interface into its own lesson.",
      dryRun: "Write the same step for a different resource with the same shape.",
      build: `Write a TypeScript type named Shift with id plus: worker, role, startsAt. Every row also needs id — the mock never shows it, but the list and the API both key off it.`,
    },
  },
  {
    id: "step3",
...
Your task: hold shifts in state that React watches, typed as Shift[], starting empty.`,
    hint: `useState<Shift[]>([]). Import useState.`,
    example_code: `const [guests, setGuests] = useState<Guest[]>([]);`,
    think_prompt: `\`\`\`text
LIST — Shifts
  Ana
  Barista   (grows by one every time publish is used)
\`\`\`
...
React only redraws a component when the value it reads changes through React itself — a plain variable can change without React ever finding out. Where should this growing array live so the screen actually redraws every time a row is added?`,
    mc_options: ["const [shifts, setShifts] = useState<Shift[]>([]);","let shifts = [];","const shifts = fetch('/api');"],
    mc_correct_option: "const [shifts, setShifts] = useState<Shift[]>([]);",
    mc_anchor: "const [shifts, setShifts] = useState<Shi",
    why_this_matters: `A plain array in a variable will not make React redraw — useState is the list the screen actually watches. Small teams still run on group chats. A shift list+form is the staffing desk they can actually use.`,
    answer_keywords: ["useState","shifts","setShifts","Shift"],
...
`,
      ...
      watchOut: "Do not turn a single import or interface into its own lesson.",
      dryRun: "Write the same step for a different resource with the same shape.",
      build: `useState<Shift[]>([]). Import useState.`,
    },
  },
  {
    id: "step4",
...
Your task: now that you have the data as an array you can iterate over, return the JSX that renders each item as a row with key={item.id}.`,
    hint: `shifts.map((a) => <li key={a.id}>...</li>)`,
    example_code: `return (
  <ul>
    {guests.map((g) => (
...
Turning an array into UI is a mapping operation — one array item becomes one rendered element — and React needs a stable identifier per element to tell rows apart across re-renders. You already have the array in state. How do you turn it into the stacked rows shown above?`,
    ...
    why_this_matters: `map() walks the array and returns one element per item; a stable key tells React which row is which when the list changes. Small teams still run on group chats. A shift list+form is the staffing desk they can actually use.`,
    answer_keywords: ["map","key","a.id","shifts"],
    seed_code: `import { useState } from "react";
...
`,
      ...
      watchOut: "Do not turn a single import or interface into its own lesson.",
      dryRun: "Write the same step for a different resource with the same shape.",
      build: `shifts.map((a) => <li key={a.id}>...</li>)`,
    },
  },
  {
    id: "step5",
...
Your task: render the empty message above when shifts.length === 0, and the mapped rows otherwise.`,
    hint: `shifts.length === 0 ? <p>No shifts yet.</p> : <ul>...</ul>`,
...
An array with zero items is a valid, common state — code that only knows how to map rows renders nothing at all when the array is empty, with no explanation for the user. When should the UI show the empty message above instead of the (empty) list?`,
    ...
    why_this_matters: `An empty list should not look broken — a clear empty state tells the user they can add the first row. Small teams still run on group chats. A shift list+form is the staffing desk they can actually use.`,
...
export function ShiftBoard() {
  ...
  return <ul>{shifts.map((a) => <li key={a.id}>{a.worker}</li>)}</ul>;
}
`,
    starter_code: `import { useState } from "react";
...
`,
      ...
      watchOut: "Do not turn a single import or interface into its own lesson.",
      dryRun: "Write the same step for a different resource with the same shape.",
      build: `shifts.length === 0 ? <p>No shifts yet.</p> : <ul>...</ul>`,
    },
  },
  {
    id: "step6",
