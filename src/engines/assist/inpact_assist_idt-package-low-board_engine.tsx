...
`,
  ...
  },
  {
    ...
    type: "question",
    phase: "Step 1 of 7",
    paal: `Every step from here on adds to one function. Write and export a function named \`LowPackageBoard\` that returns \`<div />\`.

Your task: define and export LowPackageBoard as a function component returning <div />.`,
    hint: `export function LowPackageBoard() {
  return <div />;
}`,
  ...
  return <div />;
    ...
    mc_options: ["export function LowPackageBoard() { return <div />; }","Write the JSX first, then wrap it in a function later","Skip the function — a component can be a bare object of props"],
    mc_correct_option: "export function LowPackageBoard() { return <div />; }",
    mc_anchor: "export function LowPackageBoard() { retu",
    why_this_matters: `A React component is just a function that returns JSX — naming and exporting that shell first is what lets every later step (and a real pull request) attach to something. Owners upsell when a card is almost empty. Filter for display; keep full packages in state.`,
    answer_keywords: ["export","function","LowPackageBoard","return"],
...
`,
    ...
    expected: `export function LowPackageBoard() {
  return <div />;
}
`,
  ...
  Filter   →  only matching rows render — the full list stays in state
`,
      discover: `export function LowPackageBoard() {
  return <div />;
}
`,
      ...
      build: `export function LowPackageBoard() {
  return <div />;
}`,
    },
  },
...
Your task: write \`type ServicePackage\` with \`id\` plus client, service, status.`,
    hint: `Write a TypeScript type named ServicePackage with id plus: client, service, status. Every row also needs id — the mock never shows it, but the list and the API both key off it.`,
...
Every value with a shape needs one type to describe that shape — and a list screen renders many values of the exact same shape, over and over. Looking at the mock row above, what does that shared type need to name — including a field the mock never shows on screen at all?`,
    mc_options: ["Build one type for a single ServicePackage row (id, client, service, status)","Lock every screen to final copy and branding before modeling how one row looks","Wait until every backend endpoint exists before rendering any UI"],
    mc_correct_option: "Build one type for a single ServicePackage row (id, client, service, status)",
    mc_anchor: "Build one type for a single ServicePacka",
    ...
    answer_keywords: ["export","type","ServicePackage","client","service","status"],
    seed_code: `export function LowPackageBoard() {
  return <div />;
}

// Describe one list item. The list and form will use this type.
`,
    starter_code: `export function LowPackageBoard() {
  return <div />;
}
...
`,
      ...
      watchOut: "Do not turn a single import or interface into its own lesson.",
      dryRun: "Write the same step for a different resource with the same shape.",
      build: `Write a TypeScript type named ServicePackage with id plus: client, service, status. Every row also needs id — the mock never shows it, but the list and the API both key off it.`,
    },
  },
  {
    id: "step3",
...
Your task: hold packages in state that React watches, typed as ServicePackage[], starting empty.`,
    hint: `useState<ServicePackage[]>([]). Import useState.`,
    example_code: `const [guests, setGuests] = useState<Guest[]>([]);`,
    think_prompt: `\`\`\`text
LIST — Almost empty
  Riley
  Cut   (grows by one every time sell is used)
\`\`\`
...
React only redraws a component when the value it reads changes through React itself — a plain variable can change without React ever finding out. Where should this growing array live so the screen actually redraws every time a row is added?`,
    mc_options: ["const [packages, setPackages] = useState<ServicePackage[]>([]);","let packages = [];","const packages = fetch('/api');"],
    mc_correct_option: "const [packages, setPackages] = useState<ServicePackage[]>([]);",
    mc_anchor: "const [packages, setPackages] = useState",
    why_this_matters: `A plain array in a variable will not make React redraw — useState is the list the screen actually watches. Owners upsell when a card is almost empty. Filter for display; keep full packages in state.`,
    answer_keywords: ["useState","packages","setPackages","ServicePackage"],
...
`,
      ...
      watchOut: "Do not turn a single import or interface into its own lesson.",
      dryRun: "Write the same step for a different resource with the same shape.",
      build: `useState<ServicePackage[]>([]). Import useState.`,
    },
  },
  {
    id: "step4",
...
Your task: render packages.filter(...) mapped to rows — keep the full array in state, only narrow what's displayed.`,
    hint: `packages.filter(...).map(...)`,
    example_code: `return (
  <ul>
...
Filtering for display means computing a smaller array from the full one with .filter() — the state array itself never loses any rows. How do you keep the complete packages list in state but only render the subset above?`,
    ...
    why_this_matters: `Filtering in render (or with a derived array) lets users scan what matters without deleting other rows from state. Owners upsell when a card is almost empty. Filter for display; keep full packages in state.`,
    answer_keywords: ["filter","map","packages"],
    seed_code: `import { useState } from "react";
...
EMPTY — Almost empty
  "No low-balance packages."

Your task: render the empty message above when packages.length === 0, and the mapped rows otherwise.`,
    hint: `packages.length === 0 ? <p>No low-balance packages.</p> : <ul>...</ul>`,
...
An array with zero items is a valid, common state — code that only knows how to map rows renders nothing at all when the array is empty, with no explanation for the user. When should the UI show the empty message above instead of the (empty) list?`,
    ...
    why_this_matters: `An empty list should not look broken — a clear empty state tells the user they can add the first row. Owners upsell when a card is almost empty. Filter for display; keep full packages in state.`,
...
export function LowPackageBoard() {
  ...
  return <ul>{packages.map((a) => <li key={a.id}>{a.client}</li>)}</ul>;
}
`,
    starter_code: `import { useState } from "react";
...
`,
      ...
      watchOut: "Do not turn a single import or interface into its own lesson.",
      dryRun: "Write the same step for a different resource with the same shape.",
      build: `packages.length === 0 ? <p>No low-balance packages.</p> : <ul>...</ul>`,
    },
  },
  {
    id: "step6",
