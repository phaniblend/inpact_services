...
Your task: declare \`let templates = []\` and a \`nextId()\` helper backed by a counter (not Date.now()).`,
    hint: `let templates = []; let nextIdCounter = 1;
...
A server that must remember data across requests needs one place to keep it that persists between calls, plus a way to generate a new identifier every time a record is created. Where do the rows above live between one request and the next in a simple lesson server, and how does each new row get an id nothing else already has?`,
    ...
    why_this_matters: `Two templates with the same name on one channel confuse staff. The API rejects duplicates. APIs need a single place to read and write; an array plus a real id generator is enough for the lesson, and the same shape carries over once templates becomes a database table.`,
...
`,
      ...
      build: `let templates = []; let nextIdCounter = 1;
function nextId() { return String(nextIdCounter++); }`,
    },
  },
  {
...
Your task: write validateTemplate(input) that returns an error string when a required field is missing/invalid, else null.`,
    hint: `validateTemplate(input) returns a string error or null.`,
...
Whatever a client sends in a request body can never be assumed well-formed — checking for missing or malformed fields before anything else runs is what keeps bad data from ever reaching storage. Looking at the POST body above, which checks have to pass before you push a row?`,
    ...
    why_this_matters: `Bad payloads must fail early with a clear error — validation is how /api/reminder-templates protects the store. Two templates with the same name on one channel confuse staff. The API rejects duplicates.`,
...
`,
      ...
      watchOut: "Do not turn a single import or interface into its own lesson.",
      dryRun: "Write the same step for a different resource with the same shape.",
      build: `validateTemplate(input) returns a string error or null.`,
    },
  },
  {
    id: "step3",
...
SAMPLE — /api/reminder-templates
Conflict rule: same name + channel → conflict

Your task: write hasTemplateName(candidate) that returns true when an existing row already matches the conflict rule above.`,
    hint: `hasTemplateName(candidate) returns true when an existing row conflicts.`,
...
Two requests can describe the same resource at the same moment — before inserting one, you compare it against everything already stored to decide whether it collides. Given the rule above, how should the server decide "conflict" before insert?`,
    ...
    why_this_matters: `Clients can race — the server must refuse overlaps so the record stays trustworthy. Two templates with the same name on one channel confuse staff. The API rejects duplicates.`,
...
`,
      ...
      watchOut: "Do not turn a single import or interface into its own lesson.",
      dryRun: "Write the same step for a different resource with the same shape.",
      build: `hasTemplateName(candidate) returns true when an existing row conflicts.`,
    },
  },
  {
    id: "step4",
...
Your task: wire list() to return templates, and create() to call validateTemplate (400 on error), then hasTemplateName (409 on conflict), then push and return 201.`,
    hint: `create: validate → overlap → push; list: res.json(templates)`,
...
A route handler is where validation, conflict-checking, and storage come together — it should call the helpers you already wrote, in order, not re-implement any of their logic inline. How do GET and POST above use templates, validateTemplate, and hasTemplateName together?`,
    ...
    why_this_matters: `GET returns the store; POST validates (400), checks conflict (409), then inserts (201) — keep rules in helpers, not duplicated in the route. Two templates with the same name on one channel confuse staff. The API rejects duplicates.`,
...
`,
      ...
      watchOut: "Do not turn a single import or interface into its own lesson.",
      dryRun: "Write the same step for a different resource with the same shape.",
      build: `create: validate → overlap → push; list: res.json(templates)`,
    },
  },
];
