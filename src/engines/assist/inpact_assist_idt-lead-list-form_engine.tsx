...
`,
  ...
  },
  {
    ...
    type: "question",
    phase: "Step 1 of 7",
    paal: `Every step from here on adds to one function. Write and export a function named \`LeadInbox\` that returns \`<div />\`.

Your task: define and export LeadInbox as a function component returning <div />.`,
    hint: `export function LeadInbox() {
  return <div />;
}`,
  ...
  return <div />;
    ...
    mc_options: ["export function LeadInbox() { return <div />; }","Write the JSX first, then wrap it in a function later","Skip the function — a component can be a bare object of props"],
    mc_correct_option: "export function LeadInbox() { return <div />; }",
    mc_anchor: "export function LeadInbox() { return <di",
    why_this_matters: `A React component is just a function that returns JSX — naming and exporting that shell first is what lets every later step (and a real pull request) attach to something. Leads die in DMs. A capture list+form is the light CRM screen SMBs need before they can afford a full suite.`,
    answer_keywords: ["export","function","LeadInbox","return"],
...
`,
    ...
    expected: `export function LeadInbox() {
  return <div />;
}
`,
  ...
  Submit   →  the new row appears on the list
`,
      discover: `export function LeadInbox() {
  return <div />;
}
`,
      ...
      build: `export function LeadInbox() {
  return <div />;
}`,
    },
  },
...
Your task: write \`type Lead\` with \`id\` plus name, source, note.`,
    hint: `Write a TypeScript type named Lead with id plus: name, source, note. Every row also needs id — the mock never shows it, but the list and the API both key off it.`,
...
Every value with a shape needs one type to describe that shape — and a list screen renders many values of the exact same shape, over and over. Looking at the mock row above, what does that shared type need to name — including a field the mock never shows on screen at all?`,
    mc_options: ["Build one type for a single Lead row (id, name, source, note)","Lock every screen to final copy and branding before modeling how one row looks","Wait until every backend endpoint exists before rendering any UI"],
    mc_correct_option: "Build one type for a single Lead row (id, name, source, note)",
    ...
    answer_keywords: ["export","type","Lead","name","source","note"],
    seed_code: `export function LeadInbox() {
  return <div />;
}

// Describe one list item. The list and form will use this type.
`,
    starter_code: `export function LeadInbox() {
  return <div />;
}
...
`,
      ...
      watchOut: "Do not turn a single import or interface into its own lesson.",
      dryRun: "Write the same step for a different resource with the same shape.",
      build: `Write a TypeScript type named Lead with id plus: name, source, note. Every row also needs id — the mock never shows it, but the list and the API both key off it.`,
    },
  },
  {
    id: "step3",
...
LIST — Leads
  Jordan
  Instagram   (grows by one every time capture is used)
\`\`\`
...
React only redraws a component when the value it reads changes through React itself — a plain variable can change without React ever finding out. Where should this growing array live so the screen actually redraws every time a row is added?`,
    ...
    why_this_matters: `A plain array in a variable will not make React redraw — useState is the list the screen actually watches. Leads die in DMs. A capture list+form is the light CRM screen SMBs need before they can afford a full suite.`,
...
Your task: now that you have the data as an array you can iterate over, return the JSX that renders each item as a row with key={item.id}.`,
    hint: `leads.map((a) => <li key={a.id}>...</li>)`,
    example_code: `return (
  <ul>
    {guests.map((g) => (
...
Turning an array into UI is a mapping operation — one array item becomes one rendered element — and React needs a stable identifier per element to tell rows apart across re-renders. You already have the array in state. How do you turn it into the stacked rows shown above?`,
    ...
    why_this_matters: `map() walks the array and returns one element per item; a stable key tells React which row is which when the list changes. Leads die in DMs. A capture list+form is the light CRM screen SMBs need before they can afford a full suite.`,
    answer_keywords: ["map","key","a.id","leads"],
    seed_code: `import { useState } from "react";
...
`,
      ...
      watchOut: "Do not turn a single import or interface into its own lesson.",
      dryRun: "Write the same step for a different resource with the same shape.",
      build: `leads.map((a) => <li key={a.id}>...</li>)`,
    },
  },
  {
    id: "step5",
...
Your task: render the empty message above when leads.length === 0, and the mapped rows otherwise.`,
    hint: `leads.length === 0 ? <p>No leads yet.</p> : <ul>...</ul>`,
...
An array with zero items is a valid, common state — code that only knows how to map rows renders nothing at all when the array is empty, with no explanation for the user. When should the UI show the empty message above instead of the (empty) list?`,
    ...
    why_this_matters: `An empty list should not look broken — a clear empty state tells the user they can add the first row. Leads die in DMs. A capture list+form is the light CRM screen SMBs need before they can afford a full suite.`,
...
`,
      ...
      watchOut: "Do not turn a single import or interface into its own lesson.",
      dryRun: "Write the same step for a different resource with the same shape.",
      build: `leads.length === 0 ? <p>No leads yet.</p> : <ul>...</ul>`,
    },
  },
  {
    id: "step6",
