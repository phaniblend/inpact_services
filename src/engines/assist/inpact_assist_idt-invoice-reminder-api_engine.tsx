...
A server that must remember data across requests needs one place to keep it that persists between calls, plus a way to generate a new identifier every time a record is created. Where do the rows above live between one request and the next in a simple lesson server, and how does each new row get an id nothing else already has?`,
    ...
    why_this_matters: `Do not spam: one pending reminder per invoice/channel is a server rule, not a UI hope. APIs need a single place to read and write; an array plus a real id generator is enough for the lesson, and the same shape carries over once reminders becomes a database table.`,
...
Your task: write validateReminder(input) that returns an error string when a required field is missing/invalid, else null.`,
    hint: `validateReminder(input) returns a string error or null.`,
...
Whatever a client sends in a request body can never be assumed well-formed — checking for missing or malformed fields before anything else runs is what keeps bad data from ever reaching storage. Looking at the POST body above, which checks have to pass before you push a row?`,
    ...
    why_this_matters: `Bad payloads must fail early with a clear error — validation is how /api/reminders protects the store. Do not spam: one pending reminder per invoice/channel is a server rule, not a UI hope.`,
...
`,
      ...
      watchOut: "Do not turn a single import or interface into its own lesson.",
      dryRun: "Write the same step for a different resource with the same shape.",
      build: `validateReminder(input) returns a string error or null.`,
    },
  },
  {
    id: "step3",
...
SAMPLE — /api/reminders
Conflict rule: same invoiceId + channel already pending → conflict

Your task: write hasPendingReminder(candidate) that returns true when an existing row already matches the conflict rule above.`,
    hint: `hasPendingReminder(candidate) returns true when an existing row conflicts.`,
...
Two requests can describe the same resource at the same moment — before inserting one, you compare it against everything already stored to decide whether it collides. Given the rule above, how should the server decide "conflict" before insert?`,
    ...
    why_this_matters: `Clients can race — the server must refuse overlaps so the record stays trustworthy. Do not spam: one pending reminder per invoice/channel is a server rule, not a UI hope.`,
...
`,
      ...
      watchOut: "Do not turn a single import or interface into its own lesson.",
      dryRun: "Write the same step for a different resource with the same shape.",
      build: `hasPendingReminder(candidate) returns true when an existing row conflicts.`,
    },
  },
  {
    id: "step4",
...
Your task: wire list() to return reminders, and create() to call validateReminder (400 on error), then hasPendingReminder (409 on conflict), then push and return 201.`,
    hint: `create: validate → overlap → push; list: res.json(reminders)`,
...
A route handler is where validation, conflict-checking, and storage come together — it should call the helpers you already wrote, in order, not re-implement any of their logic inline. How do GET and POST above use reminders, validateReminder, and hasPendingReminder together?`,
    ...
    why_this_matters: `GET returns the store; POST validates (400), checks conflict (409), then inserts (201) — keep rules in helpers, not duplicated in the route. Do not spam: one pending reminder per invoice/channel is a server rule, not a UI hope.`,
...
`,
      ...
      watchOut: "Do not turn a single import or interface into its own lesson.",
      dryRun: "Write the same step for a different resource with the same shape.",
      build: `create: validate → overlap → push; list: res.json(reminders)`,
    },
  },
];
