...
export default function DynamicLessonPage({
  ...
  const engineConfig = useMemo(() => {
    ...
    const base = aiLessonToEngineConfig(config, {
      track,
      language: track?.includes("ts") ? "typescript" : "javascript",
    ...
    return {
      ...
      onValidateCode: async (node, userCode) => {
        const res = await fetch("/api/lessons/validate", {
          ...
          body: JSON.stringify({
            step: {
              ...
              expectedOutcome: node.expected,
              expected: node.expected,
              seedCode: node.seed_code,
              seed_code: node.seed_code,
            },
            userCode,
        ...
        if (!res.ok) {
          ...
          const msg = err?.error || (res.status === 404 ? "Validation service not available. Run the server (npm run server) and try again." : res.status === 429 ? "Rate limit exceeded. Please try again in a moment." : res.statusText || "Validation request failed.");
          throw new Error(msg);
        }
        return res.json();
      },
